...
class Settings(BaseSettings):
    ...
    process_rank_max: int = 300             # сколько процессов ранжировать по важности (H7) за раз
    ...
    # Потолок числа узлов графа — предохранитель от патологически широких окон, а НЕ
    # рабочий лимит: темы берутся из БД (theme_id), рёбра считаются пачками по парам,
    # поэтому число процессов ничего не упирает. В обычных окнах не срабатывает.
    graph_max_processes: int = 120
    # Сколько пар-кандидатов (похожих по centroid) отдавать LLM на обоснование связей.
    # Батчатся мелко и гонятся с ограниченным параллелизмом — бюджет рёбер, не процессов.
    graph_max_edge_pairs: int = 90
