...
class Settings(BaseSettings):
    ...
    # Сколько процессов окна отдавать LLM на анализ связей. Не «жёсткий» лимит вида,
    # а бюджет промпта: все процессы попадают в промпт, а модель просят уложить их в
    # 2-6 тем при потолке ответа max_tokens. Слишком много → раздутый промпт, обрезанный
    # JSON и бессмысленные темы. Настраивается через env GRAPH_MAX_PROCESSES.
    graph_max_processes: int = 40
